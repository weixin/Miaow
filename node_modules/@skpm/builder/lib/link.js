...
function testDevMode() {
  ...
  return (0, _exec.exec)(command('read', ''), { encoding: 'utf8' }).then(({ stdout }) => (stdout || '').trim() === '1').catch(() => false) // if reading fails, assume that it's not enabled
  .then(enabled => {
    if (!enabled) {
      ...
      return new Promise((resolve, reject) => {
        yesno.ask('Do you want to enable it? (y/N)', false, ok => {
          if (ok) {
            (0, _exec.exec)(command('write', '-bool YES')).then(() => (0, _exec.exec)('/usr/bin/defaults write com.bohemiancoding.sketch3 WebKitDeveloperExtras -bool true')).then(() => console.log(`${_chalk2.default.green('success')} Sketch developer mode enabled`)).then(resolve).catch(reject);
          } else {
            resolve();
          }
        });
...
const { argv } = _yargs2.default.help().strict().usage('Usage: cd path/to/my/plugin && skpm-link');
...
try {
  ...
  testDevMode().then(() => (0, _getSketchVersion2.default)()).then(sketchVersion => {
