...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/importUIkit.js");
/******/ })
/************************************************************************/
/******/ ({
...
function importUIkitAction(context) {
  ...
  var i18 = Object(_common__WEBPACK_IMPORTED_MODULE_0__["_"])(context).importUIkit;
  ...
  if (sourceDoc.readFromURL_ofType_error(NSURL.fileURLWithPath(panel.URL().path()), "com.bohemiancoding.sketch.drawing", nil)) {
    ...
    for (var i = 0; i < sourcePages.count(); i++) {
        ...
        if (encodeURIComponent(pages[k].name()) == encodeURIComponent(sourcePageName)) {
          ...
          for (var f = 0; f < sourceSymbol.count(); f++) {
            ...
            for (var g = 0; g < localSymobl.count(); g++) {
              if (encodeURIComponent(s.name()) == encodeURIComponent(localSymobl[g].name())) {
                flagForNewSymbol = true; // 找出相同名字的，内容相同不处理，内容不同，使用source的，并把local的放到save

                if (!isSame(s, localSymobl[g])) {
                  //添加现在画布上的到冲突
                  saveArtBoard.push(localSymobl[g]); //删除画布上的
          ...
          for (var g = 0; g < localSymobl.count(); g++) {
            if (!deleteObject['g_' + g]) {
              saveArtBoard.push(localSymobl[g]); // localSymobl[g].removeFromParent();
            }
      ...
      newPage.addLayers(sourceSymbol); // newPage.addLayers(saveArtBoard2);
