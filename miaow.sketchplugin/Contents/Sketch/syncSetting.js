...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/syncSetting.js");
/******/ })
/************************************************************************/
/******/ ({
...
function syncSettingAction(context) {
  ...
  var manifestPath = context.plugin.url().URLByAppendingPathComponent("Contents").URLByAppendingPathComponent("Sketch").URLByAppendingPathComponent("config.json").path(),
      manifest = NSJSONSerialization.JSONObjectWithData_options_error(NSData.dataWithContentsOfFile(manifestPath), NSJSONReadingMutableContainers, nil); // 如果 系统key 中有，就从系统key 中读取，没有再去读取 config.json 
  ...
  var windowObject = Object(_common__WEBPACK_IMPORTED_MODULE_0__["SMPanel"])({
